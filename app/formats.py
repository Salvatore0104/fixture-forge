...
def export_ma2(f: FixtureDocument, mode_index=0):
    ...
    for ch in mode.channels:
        # Frontend stores default / highlight as percentage 0–100, same as MA2 XML.
        ...
        # ChannelFunction from/to also in percentage 0–100
        funcs = ch.functions or [type('F',(),{"name":ch.name,"dmxFrom":0,"dmxTo":100,"physicalFrom":ch.physicalFrom,"physicalTo":ch.physicalTo,"attribute":ch.attribute})()]
        for fn in funcs:
            etree.SubElement(ct, f"{{{MA_NS}}}ChannelFunction", name=fn.name, from_=str(fn.dmxFrom), to=str(fn.dmxTo), min_dmx_24=str(round(fn.dmxFrom/100*16777215)), max_dmx_24=str(round(fn.dmxTo/100*16777215)), physfrom=str(fn.physicalFrom), physto=str(fn.physicalTo), attribute=fn.attribute, feature=ch.group, preset=ch.group)
...
def import_ma2(data: bytes):
    ...
    for i, ct in enumerate(ft.xpath('.//*[local-name()="ChannelType"]')):
        ...
        # Both MA2 XML and frontend use percentage 0–100 — no scaling needed.
        funcs=[]
        for fn in ct.xpath('./*[local-name()="ChannelFunction"]'):
            funcs.append({"id":str(uuid.uuid4()),"name":fn.get('name',attr),"dmxFrom":int(fn.get('from',0)),"dmxTo":int(fn.get('to',100)),"physicalFrom":float(fn.get('physfrom',0)),"physicalTo":float(fn.get('physto',1)),"attribute":fn.get('attribute',attr)})
        channels.append({"id":str(uuid.uuid4()),"address":coarse,"attribute":attr,"group":ct.get('feature',meta['maFeature'] if meta else 'CONTROL'),"name":meta['ueAttribute'] if meta else attr,"resolution":res,"byteOrder":"MSB","defaultValue":int(ct.get('default',0)),"highlightValue":int(ct.get('highlight_value',100)),"ueAttribute":meta['ueAttribute'] if meta else attr,"functions":funcs})
