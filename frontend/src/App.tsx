...
const dmxMax=(resolution:Resolution)=>Math.pow(2,resolution)-1;
const numericValue=(value:unknown)=>typeof value==='number'?value:Number(value||0);
const clampPercent=(value:unknown)=>Math.max(0,Math.min(100,numericValue(value)));
const dmxToPercent=(value:unknown,resolution:Resolution)=>Math.round(Math.max(0,Math.min(dmxMax(resolution),numericValue(value)))/dmxMax(resolution)*100);
const normalizePercentValue=(value:unknown,resolution:Resolution)=>{
  const number=numericValue(value);
  return number>100?dmxToPercent(number,resolution):clampPercent(number);
};
const normalizeFixturePercentValues=(fixture:FixtureDocument)=>{
  const next=structuredClone(fixture);
  next.modes.forEach(mode=>mode.channels.forEach(channel=>{
    channel.defaultValue=normalizePercentValue(channel.defaultValue,channel.resolution);
    channel.highlightValue=normalizePercentValue(channel.highlightValue,channel.resolution);
  }));
  return next;
};
...
function Workbench(){
 ...
 const [mvrImportOpen,setMvrImportOpen]=useState(false);
  ...
  useEffect(()=>{
    Promise.all([api.catalog(),api.fixtures()]).then(async([c,fs])=>{
      setCatalog(c);
      const normalizedFixtures=fs.map(normalizeFixturePercentValues);
      setFixtures(normalizedFixtures);
      const first=normalizedFixtures[0];
      if(first){
        const d=await draft.get(first.id);
        select(d?normalizeFixturePercentValues(d):first);
 ...
 useEffect(()=>localStorage.setItem('fixture-forge:workspace',workspace),[workspace]);
  ...
  const importFile=async(file:File)=>{
    try{
      const f=normalizeFixturePercentValues(await api.importFile(file));
      select(f);
      setActive(f,true);
 ...
 const sortFixtureTo=(targetId:string)=>{
  ...
  const confirmMvrImport=async()=>{
    ...
    try{
      const imported=(await api.importMvr(mvrImportFile,mvrImportSelected)).map(normalizeFixturePercentValues);
  ...
  return <div className={`shell ${workspace==='mvr'?'mvr-mode':''}`}>
      ...
      <section className="table-wrap">
        <table><thead><tr><th></th><th>CH</th><th>MA 属性</th><th>自定义名称</th><th>分辨率</th><th>默认值 %</th><th>DMX 范围</th><th>属性族</th><th>操作</th></tr></thead><tbody>
          {shown.map(ch=><tr key={ch.id} className={ch.id===channelId?'selected':''} onClick={()=>selectChannel(ch.id)}>
            ...
            <td><InputNumber min={0} max={100} value={clampPercent(ch.defaultValue)} onChange={v=>patchChannelById(ch.id,{defaultValue:clampPercent(v)})}/></td>
            <td>0 – {dmxMax(ch.resolution)}</td><td><Tag color="blue">{ch.group}</Tag></td>
    ...
    {workspace==='fixture'&&<aside className="right">
      ...
      {channel?<>
        ...
        <div className="two"><label>默认值 %<InputNumber min={0} max={100} value={clampPercent(channel.defaultValue)} onChange={v=>patchChannelById(channel.id,{defaultValue:clampPercent(v)})}/></label><label>高亮值 %<InputNumber min={0} max={100} value={clampPercent(channel.highlightValue)} onChange={v=>patchChannelById(channel.id,{highlightValue:clampPercent(v)})}/></label></div>
