...
export const api={
  ...
  updateAttribute:(id:string,value:Record<string,string>)=>fetch(`/api/attributes/custom/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(value)}).then(parse),
  removeAttribute:(id:string)=>fetch(`/api/attributes/custom/${id}`,{method:'DELETE'}).then(parse),
  fixtures:()=>fetch('/api/fixtures').then(parse) as Promise<FixtureDocument[]>,
  save:(f:FixtureDocument)=>fetch(`/api/fixtures/${f.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(f)}).then(parse) as Promise<FixtureDocument>,
  create:(f:FixtureDocument)=>fetch('/api/fixtures',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(f)}).then(parse) as Promise<FixtureDocument>,
  remove:(id:string)=>fetch(`/api/fixtures/${id}`,{method:'DELETE'}).then(parse),
  validate:(id:string)=>fetch(`/api/fixtures/${id}/validate`).then(parse),
